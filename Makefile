...
install:
	...
	helm upgrade -i vcd-lb-gc charts/vcd-lb-gc --wait \
		...
		$(IMAGE_PULL_SECRETS); \
	kubectl wait --for=condition=Available deployment -l 'app.kubernetes.io/name=vcd-lb-gc,app.kubernetes.io/instance=vcd-lb-gc' --namespace=$(KUBE_NAMESPACE) --timeout=5m

.PHONY: uninstall
uninstall:
	@cd ../installer; \
	helm uninstall vcd-lb-gc --namespace=$(KUBE_NAMESPACE) || true
...
.PHONY: run
run:
	go run -mod=vendor ./cmd/vcd-lb-gc \
...
docker-certify-redhat:
	@preflight check container $(IMAGE):$(VERSION_UBI) \
		--submit \
		--certification-component-id=<vcd-lb-gc-component-id>
